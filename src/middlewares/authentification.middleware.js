const Utilisateur = require("../models/utilisateurs.model");

module.exports = (req, res, next) => {

    // Vérifier si la clé API est présente dans l'entête
    if (!req.headers.authorization) {
        return res.status(401).json({ message: "Vous devez fournir une clé api. " });
    }

    if (!req.headers.utilisateur) {
        return res.status(401).json({ message: "Vous devez fournir votre utilisateur. " });

    }

    // Récupérer la clé API
    const cleApi = req.headers.authorization.split(' ')[1];
    const utilisateur_id = req.headers.utilisateur.split(' ')[1];
    // Vérifier si la clé API est valide

    Utilisateur.validationCle(cleApi)
        .then(resultat => {
            if (!resultat) {
                return res.status(401).json({ message: "Clé API invalide. " });
            } else {
                // La clé API est valide, on continue le traitement
                Utilisateur.bonneCle(cleApi, utilisateur_id)
                    .then(utilisateur => {
                        if (!utilisateur[0]) {
                            return res.status(401).json({ message: "Mauvaise cle. " });
                        } else {
                            next();
                        }
                    })
                    .catch(erreur => {
                        return res.status(500).json({ message: "Erreur lors de la validation de l'utilisateur. " })
                    });

            }
        })
        .catch(erreur => {
            return res.status(500).json({ message: "Erreur lors de la validation de la clé api. " })
        });
}